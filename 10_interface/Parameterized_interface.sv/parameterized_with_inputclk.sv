...
module counter (count_inf inf,input clk);
  always @(posedge clk)
    begin
      if(inf.reset_n)
        inf.count <= 0;
...
// or browse Examples
interface count_inf #(parameter N=3)(input logic clk);
  logic reset_n;
  logic [N:0]count;
endinterface
...
module tb;
  ...
  counter dut(inf,clk);
   ...
   initial begin
     $monitor("clk=%0b|reset_n=%0b|count=%0d",clk,inf.reset_n,inf.count);
    $dumpvars(0,tb);
...
#    Time: 0 ns  Iteration: 0  Process: /tb/#INITIAL#19 File: testbench.sv Line: 22
